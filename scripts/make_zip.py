...
def  main():
  ...
  installer = "\installer\Comprezz Installer.exe"
   
  if demo:
    installer = "\installer\Comprezz Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Comprezz manual.pdf" 
  ]
  ...
  ZIPNAME = "Comprezz-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Comprezz-v" + FULLVERSIONSTR + "-win-demo.zip"
